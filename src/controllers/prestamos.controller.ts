...
import { prestamoService } from "../services/prestamos.services.js";
...
export async function getPrestamos(req: Request, res: Response) {
  ...
  */
  try {
    const result = await prestamoService.getPrestamosFilters(req.query);
    res.json(result);
  } catch (error: any) {
  ...
  */
  try {
    ...
    const { fecha_prestamo, fecha_devolucion, devuelto, id_socio, id_libro } =
      result.data;
    const newPrestamo = await prestamoService.createPrestamo(
      fecha_prestamo,
      fecha_devolucion,
      devuelto,
      id_socio,
