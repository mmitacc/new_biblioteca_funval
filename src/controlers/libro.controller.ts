...
export const getLibrosAll = async (req: Request, res: Response) => {
  /*  
    ...
    */
  try {
    const result = await libroService.getLibroFilter(req.query);
...
export const postLibro = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (result) {
      res
        .status(201)
        .json({ message: "Nuevo libro registrado correctamente," });
    } else {
...
export const deleteLibroId = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (listPrestamoIndexados.length !== 0) {
      res.status(404).json({
        "No se puede eliminar, esta indexado a prestamos":
          listPrestamoIndexados,
      });
      return;
    }
    ...
    if (result) {
      res
        .status(201)
        .json({ message: "El Libro fue eliminado satisfactoriamente," });
    } else {
